...
func main() {
	...
	handlers := &jobs.Handlers{
		...
		Summarizer: process.NewSummarizer(cfg.LLMAPIKeys, cfg.LLMBaseURL, cfg.LLMModel,
			...
			cfg.LLMDailyBudgetUSD, cfg.LLMMaxCallsPerHour, cfg.LLMMaxCallsPerDay,
			cfg.LLMEditorialReservePercent, db.LLM(), llmPacer),
		Telegram:          tgClient,
		Digest:            telegram.NewDigest(db, cfg.PublicBaseURL),
		TTS:               briefmedia.NewTTS(cfg.TTSAPIKeys, cfg.TTSModel, cfg.TTSVoice, ttsPacer),
		Edge:              briefmedia.NewEdgeTTS(cfg.TTSEdgeVoice, cfg.TTSEdgeProxy),
		Google:            briefmedia.NewGoogleTTS(cfg.TTSGoogleLang),
